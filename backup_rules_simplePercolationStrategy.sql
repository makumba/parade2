...
INSERT INTO `RelationQuery` VALUES (1,'SELECT f.cvsURL as fromURL, \'checkedOutAs\' as type, concat(\'file://\', concat(r.rowname, substring(f.path, length(r.rowpath) + 1 + length(r.webappPath) + case when length(r.webappPath) = 0 then 0 else 1 end, length(f.path)))) as toURL FROM File f JOIN f.row r WHERE concat(\'file://\', concat(r.rowname, substring(f.path, length(r.rowpath) + 1 + length(r.webappPath) + case when length(r.webappPath) = 0 then 0 else 1 end, length(f.path)))) = :fromURL AND f.cvsURL is not null and :percolationPath not like \'%cvs://%\'','cvs:// , checkedOutAs, file:// - all the cvs files that are a version of a repository version (not-null CVS version) and where the percolationPath has no cvs:// prefix','fromURL, percolationPath'),(4,'SELECT concat(\'file://\', concat(r.rowname, substring(f.path, length(r.rowpath) + 1 + length(r.webappPath) + case when length(r.webappPath) = 0 then 0 else 1 end, length(f.path)))) as fromURL, \'versionOf\' as type, f.cvsURL as toURL FROM File f JOIN f.row r WHERE f.cvsURL = :cvsURL AND r.rowname is not :rowName','file://, versionOf, cvs:// - all the checked out files of a cvs file that have a CVS URL set except the ones of the previous row','cvsURL, rowName'),(5,'SELECT r.fromURL as fromURL, r.type as type, r.toURL as toURL from org.makumba.devel.relations.Relation r where r.toURL = :fromURL','fromURL, type, toURL - all the files that depend on this file (through the computed relations)','');
...
-- Dump completed on 2008-06-11 19:31:45
