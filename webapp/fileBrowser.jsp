...
<c:if test="${not empty path and path != ''}"><c:set var="path" value="${path}" scope="session"/></c:if>
...
<h2 class="files">[<a href='/servlet/browse?display=file&context=${context}'>${context}</a>]/<c:forEach var="parentDir" items="${fileBrowserBean.parentDirs}"><a href='/servlet/browse?display=file&context=${context}&path=${parentDir["path"]}'>${parentDir["directoryName"]}</a>/</c:forEach><img src='/images/folder-open.gif'></h2>
<div class='pathOnDisk'>${fileBrowserBean.pathOnDisk}</div>
<table class='files'>
<tr><th></th>
<th colspan='2'>
<a href='/File.do?display=command&view=newDir&context=${context}&path=${path}' target='command' title='Create a new directory'><img src='/images/newfolder.gif' align='right'></a>
<a href='/uploadFile.jsp?context=${context}&path=${path}' target='command' title='Upload a file'><img src='/images/uploadfile.gif' align='right'></a>
<a href='/File.do?display=command&view=newFile&context=${context}&path=${path}' target='command' title='Create a new file'><img src='/images/newfile.gif' align='right'></a>
<a href='/servlet/browse?display=file&context=${context}&path=${path}&order=name' title='Order by name'>Name</a>
</th>
...
<th>CVS <a href='/Cvs.do?op=check&context=${context}&params=${path}' target='command' title='CVS check status'><img src='/images/cvs-query.gif' alt='CVS check status' border='0'></a>
<a href='/Cvs.do?op=update&context=${context}&params=${path}' target='command' title='CVS local update'><img src='/images/cvs-update.gif' alt='CVS local update' border='0'></a>
<a href='/Cvs.do?op=rupdate&context=${context}&params=${path}' target='command' title='CVS recursive update'><img src='/images/cvs-update.gif' alt='CVS recursive update' border='0'></a></th>
</tr>
