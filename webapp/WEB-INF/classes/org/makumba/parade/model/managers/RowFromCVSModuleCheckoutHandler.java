...
public class RowFromCVSModuleCheckoutHandler extends Thread {
    ...
    private Logger logger = ParadeLogger.getParadeLogger(RowFromCVSModuleCheckoutHandler.class.getName());
    ...
    public void run() {
        ...
        if (result.toString().indexOf("exit value: 1") > -1) {
            logger.severe("Could not retrieve checkout module for application " + a.getName()
            ...
            try {
                ...
                logger.severe("Could not get the canonical path for row " + r.getRowname() + " with path " + path);
            }
