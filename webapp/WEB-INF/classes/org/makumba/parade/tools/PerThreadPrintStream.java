...
public class PerThreadPrintStream extends java.io.PrintStream {
    ...
    static ThreadLocal<Object> lastEnter = new ThreadLocal<Object>() {
        @Override
    ...
    public void write(byte[] buffer, int start, int len) {
        ...
        /*
         * 
         * String s = TriggerFilter.prefix.get(); byte[] b = null; if (s != null) b = (s + ": ").getBytes(); else // if
         * s is null, it means that this didn't happen through some external access to the webapp b = ("[Thread]  " +
         * logDate.format(new Date()) + ": ").getBytes(); int lastStart = start; if (b != null) { for (int i = start; i
         * < start + len; i++) { if (buffer[i] == '\n') // FIXME, use general line separator // here. { if
         * (lastEnter.get() != null) { outWrite(b, 0, b.length); }
         * 
         * else lastEnter.set(dummy); outWrite(buffer, lastStart, i - lastStart + 1); // w.write(buffer, lastStart, i -
         * lastStart + 1); lastStart = i + 1; } } if (lastStart < start + len) { if (lastEnter.get() != null) {
         * outWrite(b, 0, b.length); }
         * 
         * lastEnter.set(null); outWrite(buffer, lastStart, start + len - lastStart); // w.write(buffer, lastStart,
         * start + len - lastStart); } } else { outWrite(buffer, start, len); // w.write(buffer, start, len); } } catch
         * (Throwable e) { PrintWriter p = new PrintWriter(new OutputStreamWriter(out)); e.printStackTrace(p);
         * p.flush(); }
         */
    }
