...
public class JNotify_linux {
    ...
    static {
        ...
        int res = nativeInit();
        if (res != 0) {
            throw new RuntimeException("Error initializing fshook_inotify library. linux error code #" + res
                    + ", man errno for more info");
        }
        init();
    }
    ...
    /*
     * All of the events - we build the list by hand so that we can add flags in the future and not break backward
     * compatibility. Apps will get only the events that they originally wanted. Be sure to add new events here!
     */
    public final static int IN_ALL_EVENT = (IN_ACCESS | IN_MODIFY | IN_ATTRIB | IN_CLOSE_WRITE | IN_CLOSE_NOWRITE
            | IN_OPEN | IN_MOVED_FROM | IN_MOVED_TO | IN_DELETE | IN_CREATE | IN_DELETE_SELF);
    ...
    static void callbackProcessEvent(String name, int wd, int mask, int cookie) {
        debug("JNotify.event(name=" + name + ", wd=" + wd + ", " + getMaskDesc(mask)
