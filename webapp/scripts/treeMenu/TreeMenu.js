...
// +----------------------------------------------------------------------+
//
// original-Id: TreeMenu.js,v 1.3 2002/06/22 14:45:36 richard Exp 
// $Id$
    ...
    */
	...
	for (var i=0; i<nodes.length; i++) {
        ...
        */
		...
		var iconimg = nodes[i].icon ? sprintf('<img src="%s/%s" align="absbottom">', this.iconpath, nodes[i].icon) : '';
        ...
        */
		...
		var imgTag    = sprintf('<img src="%s/%s%s.gif" align="absbottom" border="0" name="img_%s" %s />', this.iconpath, gifname, modifier, layerID, onMDown);
		var linkStart = nodes[i].link ? sprintf('<a class="tree" href="%s" target="%s">', nodes[i].link, this.linkTarget) : '';
		...
		if (nodes[i].n.length) {
            ...
            */
			if (parentLayerID == null && nodes.length == 1) {
				...
				var newPrepend = prepend + sprintf('<img src="%s/line.gif" align="absbottom">', this.iconpath);

			} else {
				var newPrepend = prepend + sprintf('<img src="%s/linebottom.gif" align="absbottom">', this.iconpath);
			}
